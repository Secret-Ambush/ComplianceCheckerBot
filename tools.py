...
if __name__ == "__main__":
    # Step 1: Run the first two tasks only
    partial_crew = Crew(
    ...
    partial_result = partial_crew.kickoff(inputs={
        ...
        "rules_json": open("compliance_rules.json").read()
    })

    print("✅ Initial Evaluation Result:")
    print(partial_result)

    # Step 2: Extract failed rules and explain them
    ...
    for r in rule_outcomes:
        if r["result"] == "fail":
            print(f"🔍 Explaining failure for rule: {r['rule_id']}")
